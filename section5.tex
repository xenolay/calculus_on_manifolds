...
\end{equation}によって定義した．即ち，可測関数を単関数によって下から近似することで求めた．単関数によって下から近似するとは，おおらかに言えば，空間$X$をいくつかの可測集合$\set{A_i}$に分割し，各集合$A_i$における近似値$a_i$を定めることであった．直感的には，$X$の分割$A_i$が何かしらの意味で細かくなればなるほど近似の精度が良くなっていくと思われるし，実際に\ref{非負単関数の列} の証明では，$X$の分割を細かくしていくことで，所与の可測関数に収束する単関数の列を作った．ということで，大雑把で感覚的な物言いをすると，$X$の「とても細かい分割」$\set{A_i}$が与えられているとき，$a_i \in A_i$を任意に取れば
\begin{equation}
...
\tau \coloneqq 
    ...
    \end{pmatrix}
\end{equation}で定めると，$\sgn \tau = (-1)^{k \ell}$である\footnote{いわゆる「あみだくじ」を書いて符号を計算するのが一番簡単だと思う．}．\ref{交代化作用素の基本性質} (i) を繰り返し使うと，
\begin{align}
...
\begin{proof}
$y$が標準座標系$x$の場合は$\partial / \partial x^i \coloneqq E_i$と定めればよい．それ以外の場合は，$F = f \circ x^{-1}$，$G = x \circ y^{-1}$と置いて$\left(\partial_i (f \circ y^{-1}) \right) (y (p)) = \left(\partial_i (f \circ x^{-1} \circ x \circ y^{-1}) \right) (y (p)) = \left(\partial_i (F \circ G) \right) (y (p))$に対して\ref{実用的な方の合成則} を適用すると，
\begin{align}
...
=& \begin{cases}
    \int_{[0,1]^{k-1}} \left(f \circ I^k_{(j,\alpha)}\right) \dif x^1 \wedge \dif x^2 \wedge \dots \widehat{\dif x^i} \dots \wedge \dif x^k & \text{if} j = i \\
    0 & \text{if} j \neq i
  \end{cases} \\
=& \begin{cases}
    \int_{[0,1]^{k-1}} f (x^1, x^2, \dots, \alpha, \dots, x^k)  \dif x^1 \dif x^2 \cdots \widehat{\dif x^i} \cdots \dif x^k & \text{if} j = i \\
    0 & \text{if} j \neq i
  \end{cases} \\
=& \begin{cases}
    \int_{[0,1]^{k-1}} f (x^1, x^2, \dots, \alpha, \dots, x^k)  \dif x^1 \dif x^2 \cdots \dif x^i \cdots \dif x^k & \text{if} j = i \\
    0 & \text{if} j \neq i
  \end{cases}
\end{align}
である．ゆえに
