...
//  & lt.eq sum_(i = 1)^n frac(lr(|h^i|), parallel h parallel) ∥partial_i f \( a^1 + h^1 \, dots.h \, a^(i - 1) + h^(i - 1) \, b^i \, a^(i + 1) \, dots.h \, a^n \) - partial_i f \( a \)∥\
//  $ なので，$partial_i f$の連続性より右辺は$h arrow.r 0$で0に収束する．
...
// ]
// #block[
// <開方体上の隆起関数> 関数$psi : bb(R) arrow.r bb(R)$を，
// \$\$\\psi(x) =   \\begin{dcases\*}
...
//  & = mu^(\*) (E sect union.big_(i = 1)^2 A_i sect A_1) + mu^(\*) ((E sect union.big_(i = 1)^2 A_i) \\ A_1) + mu^(\*) (E \\ union.big_(i = 1)^2 A_i)\
//  $を得るので，$B_1 = A_1 \, B_i = A_i \\ B_(i - 1)$と定めて帰納的に議論を繰り返すことで
...
// ]
// #block[
// <変数変換公式の弱い形>$A subset bb(R)^n$を開集合，$f : bb(R)^n arrow.r overline(bb(R))$を非負可測または$f in L^1 \( bb(R)^n \, cal(B)^n \, lambda^n \)$とし，$phi : A arrow.r phi \( A \)$を$C^1$級微分同相とする．このとき，
...
//      & = frac(\( k + ell + m \) !, k ! ell ! m !) "Alt" \( omega times.circle eta times.circle theta \)\
//      $となる．もう一つの等式も同様にすればよい．
...
//  & = sum_j \( partial_i G^j \) \( y \( p \) \) dot.op (frac(partial, partial x^i) f) \( p \)\
//  $が成り立つので，
...
// ]
// #block[
// <微分形式の積分はwell-defined> 上記の微分形式の積分の定義は well-defined
...
//  & = integral_(\[ 0 \, 1 \]) \( P compose c \) dot.op frac("" #h(-1em) d \( x compose c \), "" #h(-1em) d t) "" #h(-1em) d t + \( Q compose c \) dot.op frac("" #h(-1em) d \( y compose c \), "" #h(-1em) d t) "" #h(-1em) d t\
//  $ 右辺の被積分関数は$\[ 0 \, 1 \]$上で連続かつ有界，したがって特に
...
//  & = sum_(j = 1)^k sum_(alpha = 0)^1 integral_(\[ 0 \, 1 \]^(k - 1)) (f compose I_(\( j \, alpha \))^k) ((I_(\( j \, alpha \))^k)^(\*) "" #h(-1em) d x^1) and ((I_(\( j \, alpha \))^k)^(\*) "" #h(-1em) d x^2) and dots.h and ((I_(\( j \, alpha \))^k)^(\*) "" #h(-1em) d x^k)\
//  $ ところで， \$\$\\begin{aligned}
