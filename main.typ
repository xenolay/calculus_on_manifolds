...
#defi[
  $x \, y in bb(R)^n$に対し，
  $ chevron.l x \, y chevron.r colon.eq sum_(i = 1)^n x^i y^i $を対応させる写像$chevron.l dot.op \, dot.op chevron.r : bb(R)^n times bb(R)^n arrow.r bb(R)$を$bb(R)^n$の標準内積と呼ぶ．$parallel x parallel colon.eq sqrt(chevron.l x \, x chevron.r)$を$x$のノルムと呼ぶ．
...
#thm[
  $U \, V$を$bb(R)^n$の開集合とする．$U inter V$は$bb(R)^n$の開集合である．

]
#proof[
  $x in U inter V$を任意にひとつ取る．$U$は開集合なので，$x in O_U subset U$を充たすような開方体$O_U$がある．$V$も開集合なので，$x in O_V subset V$を充たすような開方体$O_V$がある．したがって$O_U inter O_V$が開方体であることが証明できればよい．$n = 1$のときは場合分けをすればあきらかである#footnote[訳：きちんと書くのがめんどくさい];し，$n gt.eq 2$の場合は各成分ごとに$n = 1$の場合の議論が適用できることから従う．
...
#proof[
  やはり開被覆は開方体のみからなっているとして一般性を失わないのでそうする；$cal(O) colon.eq { U_a times V_a }$を$A times B$の開被覆であって，$U_a$が$bb(R)^m$の開方体，$V_a$が$bb(R)^n$の開方体であるようなものとする．$x in A$をひとつとって固定する．このとき${ x } times B$はコンパクトで，$cal(O)$によって被覆されているので，この中から有限個${ U_(x_i) times V_(x_i) }$を取り出すことで${ x } times B$を被覆できる．ところで$U_x colon.eq inter.big U_(x_i)$は$x$を含む開集合になるので，${ U_x times V_(x_i) }$は${ x } times B$の開被覆になっている．この操作を各$x in A$に対し繰り返すことで，$x in A$に対して開集合$U_x subset A$とそれに付随する有限個の開集合$V_(x_i) subset B$を作ることができた．ところで$A$はコンパクトで，${ U_x }$は$A$の開被覆なので，有限部分被覆${ U_j }$が存在する．${ U_j times V_(j_i) }$は$A times B$の有限部分被覆である．
...
#proof[
  この証明では「開球が開集合である」ことは認めて使う#footnote[開球が開集合であることの証明は直接やってもよいし，連続関数を用いた証明をする場合でもこの定理は使わないので，循環論法にはならない．];．$A subset bb(R)^m$をコンパクトとする．このとき，$A$の被覆であって開方体からなるものを取ると，$A$は有限個の開方体で被覆されるので，とくに有界である．$A$が閉であること，すなわち$A^C colon.eq bb(R)^m \\ A$が開であることを示す．$x in A^C$を任意にひとつ取って固定する．$a in A$に対して$epsilon_a colon.eq parallel x - a parallel \/ 4$と定めると，$a eq.not x$なので$epsilon_a > 0$である．${ B \( a ; epsilon_a \) }$は$A$の開被覆なので，$A$のコンパクト性より有限部分被覆${ B \( i ; epsilon_i \) }$が取れる．ところで$V colon.eq B \( x ; min_i { epsilon_i } \)$は$x$をふくむ開集合であって，$V inter A = nothing$すなわち$V subset A^C$である．$x$は任意だったので，$A^C$は開集合である．
...
#thm[
  ...
  - 任意の開集合$O subset bb(R)^n$に対し，$f^(- 1) \( O \) = V inter A$を充たすような開集合$V subset bb(R)^m$が存在する．

]
<開集合の引き戻しは開集合>
#proof[
  前半から後半を示す．$O subset bb(R)^n$を開集合とする．$a in f^(- 1) \( O \)$を任意に取ると，$f \( a \) in O$である．$O$は開集合なので，充分小さい$epsilon_(f \( a \)) > 0$に対して#footnote[ここは普通であれば添字の$f \( a \)$を省いて記述するところであろうが，ここを省いてしまうと直下の改行した式が意味不明になってしまうので，重たくなるのを承知で添字をつけている．];$B \( f \( a \) ; epsilon_(f \( a \)) \) subset O$である．$f$は$A$上定義された連続関数なので，適当な$delta_(f \( a \)) > 0$に対して$f \( B \( a ; delta_(f \( a \)) \) inter A \) subset B \( f \( a \) ; epsilon_(f \( a \)) \)$すなわち$B \( a ; delta_(f \( a \)) \) inter A subset f^(- 1) \( B \( f \( a \) ; epsilon_a \) \)$が成り立つ．これを$f^(- 1) \( O \)$の各点に対して一斉に考えることで
  $ (union.big_(a in f^(- 1) \( O \)) B \( a ; delta_(f \( a \)) \)) inter A subset f^(- 1) (union.big_(a in f^(- 1) \( O \)) B \( f \( a \) ; epsilon_(f \( a \)) \)) $を得る．右辺は実際には$f^(- 1) \( O \)$に等しいし，$V colon.eq union.big_(a in f^(- 1) \( O \)) B \( a ; delta_f \( a \) \)$は開集合であるから，あとは左辺と右辺が等しいことを示せばよいが，これはあきらかである．

  後半から前半を示す．$a in A$を任意にひとつ取る．$B \( f \( a \) ; epsilon \)$は$bb(R)^n$の開集合なので，仮定より$f^(- 1) \( B \( f \( a \) ; epsilon \) \) = V inter A$を充たすような開集合$V$がある．$V$は$a$を含む開集合なので，適当な$delta$が存在して$B \( a ; delta \) subset V$である．したがって$parallel x - a parallel < delta$ならば$parallel f \( x \) - f \( a \) parallel < epsilon$である．$a$は任意であったから結論を得る．
...
#dig[
  上記の「位相空間に対する連続写像」の定義は，測度論に現れる「可測写像」の定義と酷似している一方，代数で現れる「準同型」の定義とは違う点がある．おそらくは「逆像を取る」という操作が共通部分を取る操作$inter$および和集合を取る操作$union$を保存する一方，「像を取る・写像で送る」という操作はこれらを保存しないからだろうと思っている．
...
#defi[
  ...
  + 任意の$epsilon > 0$に対して$B \( x ; epsilon \) inter A eq.not nothing$および$B \( x ; epsilon \) inter A^C eq.not nothing$の両方を充たす．
...
#exm[
  ...
  $ S \( U \) & = max_(p \, lambda^0 \, lambda) H \( p \) \
            ...
            & = max_lambda [log Z \( lambda \) + sum_(k = 1)^m lambda^k U_k] $を得る．一般に，凸関数$f : bb(R)^n arrow.r bb(R)$に対して$f^(\*) \( p \) colon.eq max_x [chevron.l x \, p chevron.r - f \( x \)]$を$f$の
  Legendre
...
#defi[
  単調増加な集合の族$A_1 subset A_2 subset dots.h.c$が$A = union.big_(i = 1)^oo A_i$を充たすことを$A_i arrow.t A$と書く．単調減少な集合の族$B_1 supset B_2 supset dots.h.c$が$B = inter.big_(i = 1)^oo B_i$を充たすことを$B_i arrow.b B$と書く．
...
#proof[
  $sigma$-加法族は集合の共通部分を取る操作について閉じている；すなわち${ cal(A)_lambda }_(lambda in Lambda)$が$sigma$-加法族からなる族であるならば，$inter.big_(lambda in Lambda) cal(A)_lambda$もまた$sigma$-加法族である．したがって$cal(C)$を含む$sigma$-加法族全体の集合を$D colon.eq { cal(A) \| cal(A) upright(" は ") cal(C) upright(" を含む ") sigma upright("-加法族") }$として，$sigma \( cal(C) \) colon.eq inter.big_(cal(A) in D) cal(A)$とおけばよい．
...
#defi[
  ...
  $A \, B in cal(P)$ならば$A inter B in cal(P)$．
...
#proof[
  ...
  well-defined
  であることを示す．$A in overline(cal(H))$をとると，$A$は排反な集合族$A_1 \, A_2 \, dots.h \, A_m in cal(H)$を用いて$A = union.big_(i = 1)^m A_i$と表されている．ここで，別の排反な集合族$B_1 \, B_2 \, dots.h \, B_n in cal(H)$を用いて$A = union.big_(j = 1)^n B_j$とも表されていたとする．このとき$1 lt.eq i lt.eq m$ならびに$1 lt.eq j lt.eq n$に対して$C_(i j) colon.eq A_i inter B_j$と置くと，${ C_(i j) }$は排反な$overline(cal(H))$の集合族であって，$union.big_(i = 1)^m C_(i j) = B_j$および$union.big_(j = 1)^n C_(i j) = A_i$を充たす．したがって$mu (union.big_(i = 1)^n A_i) = mu (union.big_(i = 1)^n union.big_(j = 1)^m C_(i j)) = mu (union.big_(j = 1)^m B_j)$を得るから$mu$は
  well-defined である．
  ...
  - $b in bb(R)$を用いて$A = \( - oo \, b \]$と書ける場合．$mu \( A \) = oo$なので$mu \( A \) gt.eq sum_(i = 1)^oo mu \( \( a_i \, b_i \] \)$はよい．$c in bb(R)$に対して$\( c \, b \] = union.big_(i = 1)^oo \( \( c \, b \] inter \( a_i \, b_i \] \)$なので，前段の結果と併せて$mu \( \( c \, b \] \) = union.big_(i = 1)^oo mu \( \( c \, b \] inter \( a_i \, b_i \] \) lt.eq union.big_(i = 1)^oo mu \( \( a_i \, b_i \] \)$を得るから$c arrow.r oo$として$mu \( A \) lt.eq sum_(i = 1)^oo mu \( \( a_i \, b_i \] \)$を得る．$A = \( a \, oo \)$ならびに$A = bb(R)$の場合も同様である．

  最後に，$mu$が$overline(cal(H))$上で可算加法的であることを示す．排反な$overline(cal(H))$の元の族${ A_i }_(i = 1)^oo$に対して$A colon.eq union.big_(i = 1)^oo A_i in overline(cal(H))$であったと仮定する．$A in overline(cal(H))$なので，$A = union.big_(j = 1)^m B_j$を充たす排反な$B_1 \, B_2 \, dots.h \, B_m in cal(H)$がある．同様に$A_i in overline(cal(H))$なので，$A_i = union.big_(k = 1)^(n_i) C_(i k)$を充たす排反な$C_(i 1) \, C_(i 2) \, dots.h \, C_(i n_i) in cal(H)$がある．$B_j inter C_(i k)$が互いに排反であることに気をつけて，
  $ mu \( A \) & = sum_(j = 1)^m mu \( B_j \) \
             & = sum_(j = 1)^m mu (union.big_(i = 1)^oo union.big_(k = 1)^(n_i) B_j inter C_(i k)) \
             & = sum_(j = 1)^m sum_(i = 1)^oo sum_(k = 1)^(n_i) mu (B_j inter C_(i k)) \
             & = sum_(i = 1)^oo sum_(j = 1)^m sum_(k = 1)^(n_i) mu (B_j inter C_(i k)) \
...
#proof[
  $\( a \, b \] in cal(H)$を任意に取ると，$\( a \, b \] = inter.big_(n = 1)^oo \( a \, b + 1 \/ n \)$と表せるので，$cal(H) subset cal(B)$すなわち$sigma \( cal(H) \) subset cal(B)$である．逆に$\( a \, b \) in cal(B)$を任意に取ると$\( a \, b \) = union.big_(n = 1)^oo \( a \, b - 1 \/ n \]$と表せるので$cal(B) subset sigma \( cal(H) \)$であるから$sigma \( cal(B) \) subset sigma \( cal(H) \)$を得る．
...
#proof[
  議論はほぼ同様なので，(i) と (ii)
  の同値性のみ示す．$cal(B) = sigma \( { \( - oo \, a \) \| a in bb(R) } \)$を示せばよい．任意の$a in bb(R)$に対して$\( - oo \, a \) in cal(B)$なので$cal(B) supset sigma \( { \( - oo \, a \) \| a in bb(R) } \)$はよい．任意の$n in bb(N)$ならびに開区間$\( a \, b \)$に対して$\( a \, b \) = inter.big_(n = 1)^oo (\( - oo \, b \) inter \( - oo \, a + 1 \/ n \)^c)$なので$\( a \, b \) in sigma \( { \( - oo \, a \) \| a in bb(R) } \)$となるから$cal(B) subset sigma \( { \( - oo \, a \) \| a in bb(R) } \)$である．
...
#defi[
  ...
  - $U \, V$が$overline(bb(R))$において開集合であるならば，$U inter V$も$overline(bb(R))$において開集合である．
...
#defi[
  ...
  $ cal(A)_1 times.o cal(A)_2 colon.eq sigma ({ A_1 times A_2 \| A_1 in cal(A)_1 \, A_2 in cal(A)_2 }) $を$cal(A)_1$と$cal(A)_2$の直積$sigma$-加法族という．$n$個の可測空間に対する直積$sigma$-加法族$cal(A)_1 times.o dots.h times.o cal(A)_n$も同様に定める．特に$cal(A)_1 = cal(A)_2 = dots.h = cal(A)_n eq.colon cal(A)$のとき，$cal(A)_1 times.o dots.h times.o cal(A)_n$を$cal(A)^n$と書く．

]
#dig[
  直積$sigma$-加法族のことを$cal(A)_1 times cal(A)_2$と書く文献も多い．これは直積集合と紛らわしい気もするが，$sigma$-加法族の直積集合を考えることはあまりないからか，一般的な記法である．他方，$cal(A)_1 times.o cal(A)_2$は（後に現れる）テンソル積と記号が衝突しているけれども，この記号衝突が混乱を引き起こすこともないと思う．個人的な所感としてはどっちもどっちなので，気分で$times.o$を使うことにしたが，$cal(A)^n$を$cal(A)^(times.o n)$とは書いていないので，お行儀が悪いと感じる人もいるかもしれない．
...
#proof[
  $n = 2$の場合を示す．
  ${ A \| A times bb(R) in cal(B) \( bb(R)^2 \) }$は$bb(R)$の開集合をすべて含む$sigma$-加法族なので，$cal(B)$を含む．従って任意の$A in cal(B)$に対して$A times bb(R) in cal(B) \( bb(R)^2 \)$が成り立つ．同様に任意の$B in cal(B)$に対して$bb(R) times B in cal(B) \( bb(R)^2 \)$が成り立つ．ゆえに任意の$A \, B in cal(B)$に対して$A times B = \( A times bb(R) \) inter \( bb(R) times B \) in cal(B) \( bb(R)^2 \)$を得るので，$cal(B)^2 subset cal(B) \( bb(R)^2 \)$．逆の包含を示すには$cal(B) = sigma \( { \( a_1 \, b_1 \) times \( a_2 \, b_2 \) \| - oo < a_i < b_i < oo \, i = 1 \, 2 } \)$を示せばよいが，これは$bb(R)^2$の開集合が開方体の可算和で書けることから従う．$n > 2$の場合の証明もほぼ同様である．
...
#proof[
  ...
  + $n = 2$の場合のみ示す．任意の$A \, B in cal(B)$に対して，$f^(- 1) \( A times B \) in cal(A)_1$は$(f^1)^(- 1) \( A \) inter (f^2)^(- 1) \( B \) in cal(A)_1$と同値であることに注意する．このことから$f^1 \, f^2$が可測ならば$f$が可測であるとわかる．逆に$f$が可測であると仮定すると，特に$B = bb(R)$とすることで$(f^1)^(- 1) \( A \) in cal(A)_1$を得るから$f^1$は可測である．$f^2$の可測性も同様である．
  ...
  + $\( inf_n f_n \)^(- 1) \( \( - oo \, a \) \) = inter.big_n \( f_n \)^(- 1) \( \( - oo \, a \) \) in cal(A)$なので$inf_n f_n$は可測である．$sup_n f_n$も同様．$liminf_n f_n = sup_n inf_(m gt.eq n) f_m$なので$liminf_n f_n$の可測性もよい．ほかも同様である．
...
#proof[
  単関数$f$が$a_i \, b_j in bb(R)$ならびに$A_i \, B_j in cal(A)$（ただし$1 lt.eq i lt.eq n$および$1 lt.eq j lt.eq m$）を用いて$f = sum_(i = 1)^n a_i 1_(A_i) = sum_(j = 1)^m b_j 1_(B_j)$と2通りに表示できていたとする．このとき$C_(i j) colon.eq A_i inter B_j$と置けば$C_(i j) in cal(A)$であり，かつ$C_(i j)$上で$a_i = b_j$なので，
...
#proof[
  $f = sum_(i = 1)^m a_i 1_(A_i)$，$g = sum_(j = 1)^n b_j 1_(B_j)$と表示し，更に$C_(i j) colon.eq A_i inter B_j$と置く．このとき$f + g = sum_(i = 1)^m sum_(j = 1)^n \( a_i + b_j \) 1_(C_(i j))$が成り立つので，
...
#proof[
  ...
  更に，次の性質を充たすような集合族$cal(C)$が$cal(A)$を含む$sigma$-加法族であることを示す；$A in cal(C)$であるならば，任意の$E in 2^X$に対して，$mu^(\*) \( E \) = mu^(\*) \( E inter A \) + mu^(\*) \( E \\ A \)$．まず$A in cal(A)$を任意に取る．すると$E subset union.big_(i = 1)^oo A_i$ならば$E inter A subset union.big_(i = 1)^oo \( A_i inter A \)$なので，
  $ {{ A_i inter A } subset cal(A) med mid(bar.v) med E subset union.big_(i = 1)^oo A_i} subset {{ B_i } subset cal(A) med mid(bar.v) med E inter A subset union.big_(i = 1)^oo B_i} $すなわち
  $ mu^(\*) \( E inter A \) lt.eq inf {sum_(i = 1)^oo mu \( A_i inter A \) med mid(bar.v) med E subset union.big_(i = 1)^oo A_i} $を得る．同様にして
  $ mu^(\*) \( E \\ A \) lt.eq inf {sum_(i = 1)^oo mu \( A_i \\ A \) med mid(bar.v) med E subset union.big_(i = 1)^oo A_i} $も従う．ゆえに
  $ mu^(\*) \( E inter A \) + mu^(\*) \( E \\ A \) & lt.eq inf {sum_(i = 1)^oo mu \( A_i \\ A \) med mid(bar.v) med E subset union.big_(i = 1)^oo A_i} + inf {sum_(i = 1)^oo mu \( A_i inter A \) med mid(bar.v) med E subset union.big_(i = 1)^oo A_i}\
  & lt.eq inf {sum_(i = 1)^oo mu \( A_i \\ A \) + sum_(i = 1)^oo mu \( A_i inter A \) med mid(bar.v) med E subset union.big_(i = 1)^oo A_i}\
  & = mu^(\*) \( E \) $を得る（最後の等式は測度の非負性，ならびに正項級数は項の順番を並べ替えても値が変わらないことから出る）．逆向きの不等式は可算劣加法性から従う．したがって$cal(A) subset cal(C)$．$X in cal(C)$ならびに$A in cal(C) arrow.r.double X \\ A in cal(C)$はよいので，$A_1 \, A_2 \, dots.h in cal(C)$のときに$union.big_(i = 1)^oo A_i in cal(C)$となること，すなわち$mu^(\*) \( E \) gt.eq mu^(\*) \( E inter union.big_(i = 1)^oo A_i \) + mu^(\*) \( E \\ union.big_(i = 1)^oo A_i \)$となることを示す（逆向きの不等式はやはり可算劣加法性から従うのでよい）．$A_1 \, A_2 in cal(C)$に対して
  $ mu^(\*) \( E \) & = mu^(\*) \( E inter A_1 \) + mu^(\*) \( E \\ A_1 \)\
  & = mu^(\*) \( E inter A_1 \) + mu^(\*) \( \( E \\ A_1 \) inter A_2 \) + mu^(\*) \( \( E \\ A_1 \)\
  & = mu^(\*) \( E inter A_1 \) + mu^(\*) \( \( E \\ A_1 \) inter A_2 \) + mu^(\*) \( \( E \\ A_1 \) \\ A_2 \)\
  & = mu^(\*) (E inter union.big_(i = 1)^2 A_i inter A_1) + mu^(\*) ((E inter union.big_(i = 1)^2 A_i) \\ A_1) + mu^(\*) (E \\ union.big_(i = 1)^2 A_i)\ $を得るので，$B_1 = A_1 \, B_i = A_i \\ B_(i - 1)$と定めて帰納的に議論を繰り返すことで
  $ mu^(\*) \( E \) = mu^(\*) (E \\ union.big_(i = 1)^n A_i) + sum_(i = 1)^n mu^(\*) (E inter B_i) $を得る．なので
  $ mu^(\*) \( E \) gt.eq mu^(\*) (E \\ union.big_(i = 1)^oo A_i) + sum_(i = 1)^n mu^(\*) (E inter B_i) $となるから，第二項に対して$n arrow.r oo$とした上で可算劣加法性を使うことで結論が従う．以上より特に任意の$A in sigma \( cal(A) \)$ならびに$E in 2^X$に対して，$mu^(\*) \( E \) = mu^(\*) \( E inter A \) + mu^(\*) \( E \\ A \)$であるとわかった．
...
#dig[
  ...
  ところで，「任意の$E in 2^X$に対して，$mu^(\*) \( E \) = mu^(\*) \( E inter A \) + mu^(\*) \( E \\ A \)$」という性質を充たす$A$に対して，「$A$は
  Carathéodory
...
#proof[
  $lambda$-システムは$sigma$-加法族と同様に共通部分をとる操作について閉じている；${ cal(L)_mu }_(mu in M)$が$lambda$-システムの族であるならば，$inter.big_(mu in M) cal(L)_mu$もまた$lambda$-システムである．そこで，$L colon.eq { cal(L) \| cal(L) upright("は") cal(P) upright("を含む") med lambda upright("-システム") }$として，$cal(L)^(\*) colon.eq inter.big_(cal(L) in L) cal(L)$が$sigma$-加法族であることを示せば証明が完結するからそうする．

  「$nothing in cal(L)^(\*)$」および「$A in cal(L)^(\*)$ならば$X \\ A in cal(L)^(\*)$」は$cal(L)^(\*)$の定め方からわかる．$cal(L)^(\*)$が$pi$-システムであることを示せば，$A \, B in cal(L)^(\*)$に対して$A union B = X \\ \( A^C inter B^C \) in cal(L)^(\*)$がわかる．このことから$cal(L)^(\*)$の族${ A_i }_(i = 1)^oo$に対して$union.big_(i = 1)^n A_i in cal(L)^(\*)$が従うので，$cal(L)^(\*)$が$lambda$-システムであることと併せて$union.big_(i = 1)^n A_i arrow.t union.big_(i = 1)^oo A_i in cal(L)^(\*)$を得る．

  以下，$cal(L)^(\*)$が$pi$-システムであることを示す．$A in cal(P)$を任意に固定し，$cal(D)_A colon.eq { B in cal(L)^(\*) \| A inter B in cal(L)^(\*) }$と置く．$cal(D)_A$が$cal(P)$を含む$lambda$-システムであることを示せば，$cal(L)^(\*) = cal(D)_A$がわかるので，$A$の任意性と併せて次のことが言える；任意の$A in cal(P)$および$B in cal(L)^(\*)$に対して$A inter B in cal(L)^(\*)$．その上で今度は$B in cal(L)^(\*)$を任意に固定し，$cal(E)_B colon.eq { A in cal(L)^(\*) \| A inter B in cal(L)^(\*) }$と置いて，$cal(E)_B$が$cal(P)$を含む$lambda$-システムであることを示せば，$cal(L)^(\*) = cal(E)_B$がわかるので，$B$の任意性と併せて$cal(L)^(\*)$が$pi$-システムであることが従う．

  議論は同様なので$cal(D)_A$が$cal(P)$を含む$lambda$-システムであることだけを示す．$cal(P)$は$pi$-システムなので$cal(P) subset cal(D)_A$である．$A in cal(L)^(\*)$だから$X in cal(D)_A$もよい．$B \, C in cal(D)_A$を$B subset C$を充たすようにとれば，$A inter \( C \\ B \) = \( A inter C \) \\ \( A inter B \) in cal(L)^(\*)$となるので$C \\ B in cal(D)_A$である．集合の増大列${ B_i }$，$B_i in cal(D)_A$を取ると，$A inter union.big_(i = 1)^n B_i in cal(L)^(\*)$なので$A inter union.big_(i = 1)^n B_i arrow.t A inter union.big_(i = 1)^oo B_i in cal(L)^(\*)$であるから$union.big_(i = 1)^oo B_i in cal(D)_A$である．

]
#proof[
  $nu_1 \, nu_2$を$mu$の拡張とし，$A in cal(A)$を$mu \( A \) < oo$を充たすように任意に1つ取って固定する．その上で，
  $ cal(L)_A colon.eq { B in sigma \( cal(A) \) \| nu_1 \( A inter B \) = nu_2 \( A inter B \) } $と置く．$cal(L)_A$が$cal(A)$を含む$lambda$-システムであることが示せたとすると，$pi$-$lambda$定理によって$sigma \( cal(A) \) = cal(L)_A$を得る．$A$は$mu \( A \) < oo$を満たせば任意だったので，$mu \( A \) < oo$を充たす任意の$A in cal(A)$ならびに任意の$B in sigma \( cal(A) \)$に対して$nu_1 \( A inter B \) = nu_2 \( A inter B \)$を得る．$mu$は$cal(A)$上$sigma$-有限なので，$mu \( A_i \) < oo$を充たす集合の増大列$A_i$があり$union.big_(i = 1)^oo A_i = X$である．したがって任意の$B in sigma \( cal(A) \)$に対して$B inter A_i arrow.t B$であるから，$nu_1 \( B \) = lim_(i arrow.r oo) nu_1 \( B inter A_i \) lim_(i arrow.r oo) nu_2 \( B inter A_i \) = nu_2 \( B \)$すなわち$nu_1 = nu_2$を得る．
...
以下暫く，$\( X_1 \, cal(A)_1 \, mu_1 \)$および$\( X_2 \, cal(A)_2 \, mu_2 \)$を$sigma$-有限な測度空間とする．$cal(A) colon.eq { A_1 times A_2 \| A_1 in cal(A)_1 \, A_2 in cal(A)_2 }$と置く．すでに定義したように，$cal(A)_1 times.o cal(A)_2 = sigma \( cal(A) \)$である．

#thm[
  $mu : cal(A) arrow.r \[ 0 \, oo \]$を$mu \( A_1 times A_2 \) colon.eq mu_1 \( A_1 \) mu_2 \( A_2 \)$で定める．このとき，$mu$は$cal(A)_1 times.o cal(A)_2$上の測度に一意的に拡張できる．
...
#defi[
  上記で得られた$cal(A)_1 times.o cal(A)_2$上の測度を$mu_1$と$mu_2$の直積測度といい，$mu_1 times mu_2$と書く．また，上記の構成を帰納的に繰り返すことで，$n$個の$sigma$-有限測度空間$\( X_1 \, cal(A)_1 \, mu_1 \) \, \( X_2 \, cal(A)_2 \, mu_2 \) \, dots.h.c \, \( X_n \, cal(A)_n \, mu_n \)$に対して$mu_i$たちの直積測度$mu_1 times dots.h.c times mu_n : cal(A)_1 times.o dots.h.c times.o cal(A)_n arrow.r \[ 0 \, oo \]$を，$mu_1 times dots.h.c times mu_n \( A_1 times dots.h.c times A_n \) = product_(i = 1)^oo mu_i \( A_i \)$を充たすように一意に定めることができる．特に$\( X_1 \, cal(A)_1 \, mu_1 \) = \( X_2 \, cal(A)_2 \, mu_2 \) = \( X_n \, cal(A)_n \, mu_n \) = \( bb(R) \, cal(B) \, lambda \)$のとき，$\( bb(R)^n \, cal(B)^n \)$上の直積測度$lambda^n colon.eq lambda times dots.h.c times lambda$のことを$n$次元
  Lebesgue 測度という．
...
#thm[
  ...
  + $f in L^1 \( X_1 times X_2 \, cal(A)_1 times.o cal(A)_2 \, mu_1 times mu_2 \)$に対して，以下が成り立つ．
...
#proof[
  + $cal(A)_1 times.o cal(A)_2$の元の指示関数に対して示せたならば，単調収束定理と補題
    @非負単関数の列
    を用いることで一般の非負可測関数に対しても結論が従うので，以下では$f$が指示関数の場合を示す．
    $ cal(C) colon.eq { A in cal(A)_1 times.o cal(A)_2 \| 1_A upright("に対して (i-a), (i-b), (i-c) が成り立つ") } $と置いて，$cal(C) supset cal(A)_1 times.o cal(A)_2$を示す．$pi$-$lambda$定理を踏まえれば，次の3点を示せば充分である．
    ...
    - $E \, F in cal(C)$かつ$E subset F$ならば$F \\ E in cal(C)$となることを確認する．まず$mu \( X \) \, mu \( Y \) < oo$となる場合を見よう．$1_(F \\ E) = 1_F - 1_E$なので
      ...
      (i-c)
      も従う（ここで$mu \( Y \) < oo$なのでやはりこの過程で未定義な値は現れない）．$mu \( X \) = oo$ないしは$mu \( Y \) = oo$の場合は，$X$と$Y$の$sigma$-有限性より$A_i arrow.t X$および$B_i arrow.t Y$となる測度有限な集合の可算列${ A_i }$および${ B_i }$がとれる．$E$，$F$をそれぞれ$E inter A_i times B_i$，$F inter A_i times B_i$に置き換えれば上記の考察がそのまま適用できるので，適用した上で単調収束定理を用いればよい．
  ...
  + $f = f^(+) - f^(-)$と分解すれば，$f^(+)$および$f^(-)$は非負可測なので
    ...
    に定義された$cal(A)_2 \/ cal(B)$-可測関数である#footnote[主に私自身の備忘のため，この部分についてもう少し述べておこう．$x^2 mapsto integral f^(+) d mu_1 \( x^1 \)$が有限値を取らない$x^2$の集合を$N_(+)$，$x^2 mapsto integral f^(-) d mu_1 \( x^1 \)$が有限値を取らない$x^2$の集合を$N_(-)$と置く．$x^2 in N_(+) inter N_(-)$である場合$x^2 mapsto integral f \( x^1 \, x^2 \) d mu \( x^1 \)$は定義できないが，$mu_1 \( N_(+) \) = mu_1 \( N_(-) \) = 0$であるので，特に$mu \( N_(+) inter N_(-) \) = 0$である．従って$x mapsto integral f \( x^1 \, x^2 \) d mu \( x^1 \)$は$mu_2$-a.e.
...
#proof[
  有理点$q in A$を任意に取ると，各辺の長さがすべて等しい有界閉方体$U_q$であって，$q in U_q subset A$を充たすものがある．${ U_q }_(q in A)$は$A$の可算被覆であるから，各$U_q$に対して$lambda^n \( phi \( U_q inter B \) \) = 0$を示せば，$union.big_(q in A) phi \( U_q inter B \) = phi \( union.big_(q in A) U_q inter B \) = phi \( B \)$と併せて結論を得るのでそうする．以降，$U_q$の添字を省いて単に$U$と書く．
  ...
  $x in S inter B$を取ると，$det J_phi \( x \) = 0$なので，$d phi_x$は正則ではない．したがって${ d phi_x \( y - x \) \| y in S }$はある$n - 1$次元部分空間$V subset.neq bb(R)^n$に含まれる．更に$y in S$に対して$f \( y \) colon.eq d phi_x \( y \) - phi \( y \)$と定めると，$parallel phi_i f^j \( y \) parallel = parallel partial_i phi^j \( x \) - partial_i phi^j \( y \) parallel < epsilon \/ n^2$なので，@微分の定量評価
  より$parallel d phi_x \( y - x \) - phi \( y \) + phi \( x \) parallel = parallel f \( y \) - f \( x \) parallel < epsilon parallel y - x parallel lt.eq epsilon ell sqrt(n) \/ N$である．言い換えると，$d \( V + phi \( x \) \, phi \( y \) \) < epsilon ell sqrt(n) \/ N$である．また$phi$の$S$上での一様連続性より$parallel phi \( y \) - phi \( x \) parallel < M parallel y - x parallel lt.eq M ell sqrt(n) \/ N$となるような$M > 0$がある．したがって（$epsilon \, N$の何れにも依存しない）定数$A > 0$があって$lambda^n \( { phi \( y \) \| y in S } \) < A \( M ell sqrt(n) \/ N \)^(n - 1) dot.op epsilon ell sqrt(n) \/ N = A M^(n - 1) epsilon \( ell sqrt(n) \/ N \)^n$が成り立つ．$S$は$S inter B = nothing$を充たすように$N^n$個の小閉方体から任意に選んできたものだったので，結局$lambda^n \( { phi \( y \) \| y in U inter S } \) < A M^(n - 1) epsilon \( ell sqrt(n) \)^n$が成り立つ．$epsilon$は任意だったので，$lambda^n \( U inter B \) = 0$である．

]
#dig[
  上記の Sard
  の定理の証明は（自分で書いておいて何なのだが）あまりわかりやすいとは思わない．アイデアとしては$U inter B$の測度を$O \( epsilon \)$で抑えるために$epsilon$に依存する項を出すところ，言い換えると「$phi \( S inter B \)$は$V$の法線方向には$O \( epsilon \)$程度しか伸びないこと」が本質的である．定理の仮定が効くのはこの部分であり，それ以外の議論はこの部分を見越して逆算している部分も多い．
...
#que[
  $bb(R)^n$が「縦ベクトルからなる空間」だとすれば$\( bb(R)^n \)^(\*)$は「横ベクトルからなる空間」であると捉えられないこともない，ということを下支えするのが以下の命題である；$x in bb(R)^n$に対し，$phi_x in \( bb(R)^n \)^(\*)$を$phi_x \( y \) colon.eq chevron.l x \, y chevron.r$で定める．写像$T : bb(R)^n in.rev x mapsto phi_x in \( bb(R)^n \)^(\*)$は線型同型写像である．ちなみにこの同型写像は標準内積に依存しているので，特に基底に依存して定まる同型写像である．その意味で「自然ではない」という言い方がされることがあるが，私は自然性のことをすべて忘れました．
...
#que[
  ...
  + $H$を Hilbert
    空間とする．$x in H$に対し，$phi_x in H^(\*)$を$phi_x \( y \) colon.eq chevron.l x \, y chevron.r$で定める．写像$T : H in.rev x mapsto phi_x in H^(\*)$は線型同型写像である（Riesz
    の表現定理）．
...
#dig[
  このノートでは Spivak
  にならって線型空間としてのテンソル積を構成することはせず，多重線型写像としてしか扱わない．実際にはふたつの線型空間$V \, W$のテンソル積と呼ばれる線型空間$V times.o W$を定義して，その空間の元をテンソルと呼ぶのが行儀良いやり方である．のだが，テンソル積は構成がめんどくさいのに加えて，構成よりも普遍性のほうが大事だと言われている（し，私もそう思う）．それにこのノートの範囲で普遍性の話を出してもあんまり得るものがなさそうな気がした上，Spivak
  のとおりにやっても Riemann
  計量や微分形式の定義には困らない．なので，Spivak
  のやり方にならうことにした．ちなみに$V times.o V$のことは$cal(T)^2 \( V \)$とも書く．このノートで$cal(T)^k \( V \)$が出てこないのはそういうわけである．

]
#defi[
  $S in cal(T)^k \( V^(\*) \)$および$T in cal(T)^ell \( V^(\*) \)$に対して，そのテンソル積$S times.o T in cal(T)^(k + ell) \( V^(\*) \)$を，
  $ S times.o T \( v_1 \, dots.h \, v_k \, v_(k + 1) \, dots.h \, v_(k + ell) \) colon.eq S \( v_1 \, dots.h \, v_k \) dot.op T \( v_(k + 1) \, dots.h \, v_(k + ell) \) $で定める．

]
#que[
  $times.o$は双線型かつ結合的である．すなわち，$S \, S_1 \, S_2 in cal(T)^k \( V^(\*) \)$および$T \, T_1 \, T_2 in cal(T)^ell \( V^(\*) \)$，$U in cal(T)^m \( V^(\*) \)$，$a in bb(R)$に対して，

  - $\( S_1 + S_2 \) times.o T = S_1 times.o T + S_2 times.o T$

  - $S times.o \( T_1 + T_2 \) = S times.o T_1 + S times.o T_2$

  - $\( a S \) times.o T = S times.o \( a T \) = a \( S times.o T \)$

  - $S times.o \( T times.o U \) = \( S times.o T \) times.o U$
...
#prop[
  ...
  $ { w^(i_1) times.o dots.h times.o w^(i_k) \| 1 lt.eq i_1 \, dots.h i_k lt.eq n } $は$cal(T)^k \( V^(\*) \)$の基底である．したがって$cal(T)^k \( V^(\*) \)$は$n^k$次元である．

]
#proof[
  $w^(i_1) times.o dots.h times.o w^(i_k)$たちが一次独立であることと$cal(T)^k \( V^(\*) \)$の生成系であることを示せばよい．まず一次独立性を示す．
  $ sum_(1 lt.eq j_1 \, dots.h \, j_k lt.eq n) a_(j_1 \, dots.h \, j_k) w^(j_1) times.o dots.h times.o w^(j_k) = 0 $が成り立っていたと仮定する．このとき左辺の線型写像を$\( v_(i_1) \, dots.h \, v_(i_k) \)$（$1 lt.eq i_1 \, dots.h \, i_k lt.eq n$）に作用させれば$a_(i_1 \, dots.h \, i_k) = 0$が出るので，一次独立性がわかる．
  ...
  & = sum_(j_1) a_1^(j_1) a_2^(j_2) dots.h a_n^(j_n) w^(j_1) times.o dots.h times.o w^(j_n) $となるので，$w^(i_1) times.o dots.h times.o w^(i_k)$たちは$cal(T)^k \( V^(\*) \)$の生成系である．
...
#exm[
  ...
  - $bb(R)^n$の標準内積$chevron.l dot.op \, dot.op chevron.r : bb(R)^n times bb(R)^n arrow.r bb(R)$は$cal(T)^2 \( \( bb(R)^n \)^(\*) \)$の元である．ところで，内積は単にテンソルであるというだけではなくて引数入れ替えに関して対称という性質がある．
...
#defi[
  ...
  - $omega in and.big^k \( V^(\*) \)$，$eta in and.big^ell \( V^(\*) \)$に対して，その外積$omega and eta in and.big^(k + ell) \( V^(\*) \)$を，
    $ omega and eta colon.eq frac(\( k + ell \) !, k ! ell !) "Alt" \( omega times.o eta \) $で定める．こちらも頭についている係数の所以は後で説明する．

]
#que[
  $omega in and.big^k \( V^(\*) \)$，$eta in and.big^ell \( V^(\*) \)$に対して，$omega times.o eta in and.big^(k + ell) \( V^(\*) \)$とは限らない．
...
#prop[
  ...
  + 外積を取る操作は結合的である．特に，$omega in and.big^k \( V^(\*) \)$，$eta in and.big^ell \( V^(\*) \)$，$theta in and.big^m \( V^(\*) \)$に対して，$\( omega and eta \) and theta = omega and \( eta and theta \) = frac(\( k + ell + m \) !, k ! ell ! m !) "Alt" \( omega times.o eta times.o theta \)$が成り立つ．

]
#proof[
  + $"Alt"$の線型性と$times.o$の双線型性よりよい．細かいことはめんどくさいのでさぼりました．
  ...
  + 証明を3段階に分割する．
    ...
    + $S in cal(T)^k \( V^(\*) \)$並びに$T in cal(T)^ell \( V^(\*) \)$に対して$"Alt" \( S \) = 0$または$"Alt" \( T \) = 0$ならば$"Alt" \( S times.o T \) = 0$となることを示す．議論は同様なので$"Alt" \( S \) = 0$の場合のみ証明する．ここでは$frak(S)_k$を（$sigma in frak(S)_k$は$k + 1 \, dots.h \, k + ell$を動かさないような$frak(S)_(k + ell)$の元だとみなすことで）$frak(S)_(k + ell)$の部分集合とみなす．
      $
      ...
      & = 0 . $ここで，$frak(S)_k inter frak(S)_k tau = nothing$が次のようにしてわかる．そうでないとして，$sigma_0 in frak(S)_k inter frak(S)_k tau$を取ると，$sigma_0 = sigma_1 tau$となるような$sigma_1 in frak(S)_k$がある．したがって$tau = \( sigma_1 \)^(- 1) sigma_0 in frak(S)_k$となるが，これは$tau$の定め方に反する．同様に，相異なる$tau_1 \, tau_2 in frak(S)_(k + ell) \\ frak(S)_k$を取ると$frak(S)_k tau_1 inter frak(S)_k tau_2 = nothing$であることもわかる．以上より，総和$sum_(sigma in frak(S)_(k + ell))$は$sum_(sigma in frak(S)_k) + sum_(tau in frak(S)_(k + ell) \\ frak(S)_k) sum_(sigma in frak(S)_k tau)$の形に分解できる．分解した各々の項が0であることはすでに見たので，全体の総和も0である．

    + $S in cal(T)^k \( V^(\*) \) \, T in cal(T)^ell \( V^(\*) \) \, U in cal(T)^m \( V^(\*) \)$に対して$"Alt" \( S times.o T times.o U \) = "Alt" \( "Alt" \( S times.o T \) times.o U \) = "Alt" \( S times.o "Alt" \( T times.o U \) \)$となることを示す．$"Alt"$は冪等なので，$"Alt" \( "Alt" \( S times.o T \) - S times.o T \) = 0$であるから，前段の結果より$"Alt" \( "Alt" \( S times.o T \) times.o U - S times.o T times.o U \) = 0$を得る．他の等式も同様である．

    + 命題を証明する．
      $ \( omega and eta \) and theta & = frac(\( k + ell \) !, k ! ell !) "Alt" \( omega times.o eta \) and theta\
      & = frac(\( k + ell \) !, k ! ell !) frac(\( k + ell + m \) !, \( k + ell \) ! m !) "Alt" \( "Alt" \( omega times.o eta \) times.o theta \)\
      & = frac(\( k + ell + m \) !, k ! ell ! m !) "Alt" \( omega times.o eta times.o theta \)\ $となる．もう一つの等式も同様にすればよい．
...
#proof[
  $ w^(i_1) and dots.h and w^(i_k) & = k ! "Alt" \( w^(i_1) times.o dots.h times.o w^(i_k) \)\
  & = sum_(sigma in frak(S)_k) \( "sgn" sigma \) w^(sigma \( i_1 \)) times.o dots.h times.o w^(sigma \( i_k \)) $であり，$sigma eq.not upright(i d)$であるならば$\( "sgn" sigma \) w^(sigma \( i_1 \)) times.o dots.h times.o w^(sigma \( i_k \)) \( v_(i_1) \, dots.h \, v_(i_k) \) = 0$であるからよい．
...
#que[
  いっぽう，定数倍の差が理論に本質的な影響を与えるかというと，（少なくともこのノートの範囲では）与えないと思う．実際，$omega and eta colon.eq "Alt" \( omega times.o eta \)$と定義する流儀もある．このようにした場合，直感的には$v_1^(\*) and v_2^(\*) \( v_1 \, v_2 \)$が$v_1$と$v_2$のなす単体の面積をはかっていると思える．森田茂之「微分形式の幾何学」曰くは特性類の一般論を述べるときなどはこちらの係数のほうが都合が良いらしい．たぶん適当な意味での自然性も充たすのだと思う．

  他にも$omega and eta colon.eq sqrt(frac(\( k + ell \) !, k ! ell !)) "Alt" \( omega times.o eta \)$と定義しても結合則は保たれる．これにも何かしらの幾何学的な意味付けを考えることができるのだと思う．
...
#proof[
  ...
  $ omega eq.colon sum_(i_1 \, dots.h \, i_k) a_(i_1 \, dots.h \, i_k) w^(i_1) times.o dots.h times.o w^(i_k) $と展開できる．両辺に交代化作用素を施すことで，
  $ "Alt" \( omega \) = omega & = sum_(i_1 \, dots.h \, i_k) a_(i_1 \, dots.h \, i_k) "Alt" \( w^(i_1) times.o dots.h times.o w^(i_k) \)\
...
#defi[
  ...
  $\( p \, v \) in T_p bb(R)^n$のことを$"Tan"_p \( v \)$とも書く．また，$T_p bb(R)^n$上の標準内積$chevron.l dot.op \, dot.op chevron.r_p$を，$chevron.l "Tan"_p \( v \) \, "Tan"_p \( v \) chevron.r_p colon.eq chevron.l v \, w chevron.r$によって定める．$\[ "Tan"_p \( e_1 \) \, dots.h \, "Tan"_p \( e_n \) \]$を$T_p bb(R)^n$の自然な向きと呼ぶ．
...
#defi[
  ...
  ベクトル場$X$に対して，$X \( p \)$のことを$X_p$とも書く．また，$X_p = "Tan"_p \( v \)$であるような$v$のことを$"Vect" \( X_p \)$であらわす．$X \, Y$をベクトル場とするとき，これらの和，関数倍，標準内積を，$\( X + Y \)_p colon.eq X_p + Y_p$，$\( f X \)_p colon.eq f \( p \) X_p$，$chevron.l X \, Y chevron.r \( p \) colon.eq chevron.l X_p \, Y_p chevron.r_p$によって定める．
...
#defi[
  $M subset bb(R)^n$の各点$p$が次の条件を充たすとき，$M$は$k$次元（微分可能）多様体であると言われる；$p$を含む開集合$U_p subset bb(R)^n$，および開集合$V_p subset bb(R)^n$ならびに微分同相写像$h_p : U_p arrow.r V_p$が存在して，$h_p \( U_p inter M \) = V_p inter (bb(R)^k times { 0 })$，すなわち$h_p \( U_p inter M \) = { y in V_p \| y^(k + 1) = dots.h = y^n = 0 }$が成り立つ．
...
#proof[
  @陰関数定理の言い換え
  より，各点$p in g \( M \)$に対して，$g \( p \)$を含む開集合$U_p$上で定義された微分同相$F_p : U_p arrow.r bb(R)^n$であって，$g compose F_p^(- 1) \( p \) = \( p^(n - k + 1) \, dots.h \, p^n \)$を充たすようなものがある．$F_p \( U_p \) colon.eq V_p$と置く．$F_p : U_p arrow.r V_p$が，定義の条件にあるような微分同相写像であること，すなわち$F_p \( U_p inter M \) = V_p inter (bb(R)^(n - k) times { 0 })$を示せばよい．

  $F_p \( U_p inter M \) subset V_p inter (bb(R)^(n - k) times { 0 })$を示すために，$q in U_p inter M$を任意に取って$r colon.eq F_p \( q \)$と置く．$r in V_p$はよい．$g compose F_p^(- 1) \( r \) = g \( p \) = 0$なので，$r^(n - p + 1) = dots.h = r^n = 0$，すなわち$r in bb(R)^(n - k) times { 0 }$がわかる．

  逆の包含を示すために$r in V_p inter (bb(R)^(n - k) times { 0 })$を取る．$F_p^(- 1) \( r \) in U_p$はよい．$g compose F_p^(- 1) \( r \) = 0$なので$F_p^(- 1) \( r \) in M$である．
...
#proof[
  $M$が$k$次元多様体であったとする．$p in M$を任意に取れば，定義にあるような微分同相$h_p : U_p arrow.r V_p$が存在する．このとき，$W_p colon.eq { \( b^1 \, dots.h \, b^k \) \| b in V_p inter \( bb(R)^k times { 0 } \) }$として，$f : W_p arrow.r bb(R)^n$を$f \( a \) colon.eq h_p^(- 1) \( a \, 0 \)$で定める．$h_p$が$C^oo$級微分同相であることから$f$の単射性と$C^oo$級であることがわかる．$W_p subset bb(R)^k$が開集合であることはよく，$\( a \, 0 \) in V_p inter \( bb(R)^k times { 0 } \)$なので$f \( a \) = h_p^(- 1) \( a \, 0 \) in U_p inter M$である．更に$f^(- 1) = h_p \|_(f \( W_p \))$なので（連続関数の制限は常に連続なのだから）$f^(- 1)$は連続である．最後に，関数$H_p : bb(R)^n arrow.r bb(R)^k$を$H_p \( a \) colon.eq \( h_p^1 \( a \) \, dots.h \, h_p^k \( a \) \)$によって定めれば，$H_p$は微分可能であり，更に$H_p \( f \( q \) \) = q$を充たす．したがって
  Jacobi
  行列を考えることで$J_(H_p) \( f \( q \) \) dot.op J_f \( q \) = I_k$がわかるので，特に$J_f \( q \)$の階数は$k$である．

  逆に$M$の各点が座標条件を充たすと仮定する．$p in M$を任意に取れば，座標条件にあらわれる関数$f : W_p arrow.r bb(R)^n$が存在する．$p = f \( q \)$によって$q in bb(R)^k$を定める．関数$g : W_p times bb(R)^(n - k) arrow.r bb(R)^n$を，$g \( a \, b \) colon.eq f \( a \) + \( 0 \, b \)$で定めると，$det D_g \( q \, 0 \) = det D_f \( q \) eq.not 0$であることから，逆関数定理より$\( q \, 0 \)$を含む開集合$V'$ならびに$p$を含む開集合$U'$が存在して，$g$は$V'$上で$C^oo$級の逆写像$h_p : U' arrow.r V'$を持つ．また，$f$が座標条件を充たすことから，${ f \( a \) \| \( a \, 0 \) in V' } = U'' inter M$を充たす開集合$U''$が存在する#footnote[この部分は行間が大きいと思うが，本文に組み込むと議論の見通しが悪くなるので脚注で補う；座標条件の
    (i)
    より，$f \( W_p \) = U_1 inter M$となるような開集合$U_1$がある．更に座標条件の
    (iii)
    より$f^(- 1)$は連続であるから，$W' colon.eq { a \| \( a \, 0 \) in V' }$は開集合であることとあわせれば@開集合の引き戻しは開集合
    より${ f \( a \) \| \( a \, 0 \) in V' } = f \( W' \) = U_2 inter f \( W_p \)$となるような開集合$U_2$がある．$U'' colon.eq U_1 inter U_2$とすればよい．];．$U_p colon.eq U' inter U''$，$V_p colon.eq g^(- 1) \( U_p \)$とする．このとき，$U_p inter M = { f \( a \) \| \( a \, 0 \) in V_p } = { g \( a \, 0 \) \| \( a \, 0 \) in V_p }$なので#footnote[この部分も行間があるので補う．2つ目の等号はよいと思うので，$U_p inter M = { g \( a \, 0 \) \| \( a \, 0 \) in V_p }$を示す．$V_p subset V'$なので，${ g \( a \, 0 \) \| \( a \, 0 \) in V_p } subset { g \( a \, 0 \) \| \( a \, 0 \) in V' } = U'' inter M$であり，かつ${ g \( a \, 0 \) \| \( a \, 0 \) in V_p } subset g \( V' \) = U'$であるから，${ g \( a \, 0 \) \| \( a \, 0 \) in V_p } subset U' inter U'' inter M = U_p inter M$である．逆の包含を示すために$p in U_p inter M$を取る．$p in.not { g \( a \, 0 \) \| \( a \, 0 \) in V_p }$であったとすると，$p in { g \( a \, 0 \) \| \( a \, 0 \) in V' }$なので，結局$p in { g \( a \, 0 \) \| \( a \, 0 \) in V' \\ V_p }$であることになるが，これは$p in U_p = g \( V_p \)$に反する．];，$h_p \( U_p inter M \) = g^(- 1) \( U_p inter M \) = g^(- 1) \( { g \( a \, 0 \) \| \( a \, 0 \) in V_p } \) = { \( a \, 0 \) \| \( a \, 0 \) in V_p } = V_p inter \( bb(R)^k times { 0 } \)$である．
...
#defi[
  ...
  + $p$を含む開集合$U_p subset bb(R)^n$，および開集合$V_p subset bb(R)^n$ならびに微分同相写像$h_p : U_p arrow.r V_p$が存在して，$h_p \( U_p inter M \) = V_p inter (bb(R)^k times { 0 })$，すなわち$h_p \( U_p inter M \) = { y in V_p \| y^(k + 1) = dots.h = y^n = 0 }$が成り立つ．

  + $p$を含む開集合$U_p subset bb(R)^n$，および開集合$V_p subset bb(R)^n$ならびに微分同相写像$h_p : U_p arrow.r V_p$が存在して，$h_p \( U_p inter M \) = V_p inter (bb(H)^k times { 0 })$，すなわち$h_p \( U_p inter M \) = { y in V_p \| y^k gt.eq 0 \, y^(k + 1) = dots.h = y^n = 0 }$が成り立ち，更に$h_p \( p \)$の第$k$成分は0である．
...
#prop[
  上記の定義は well-defined
  である；$c_1 \, c_2 : \[ 0 \, 1 \]^k arrow.r M$を，向きを変えない曲$k$方体とする．更に$omega$を$M$上の$k$形式であって，$c_1 \( \[ 0 \, 1 \]^k \) inter c_2 \( \[ 0 \, 1 \]^k \)$の外部で0であるとする．このとき，$integral_(c_1) omega = integral_(c_2) omega$が成り立つ．
...
#prop[
  ...
  $ chevron.l z \, w chevron.r = det thin \( v_1 \, dots.h \, v_(n - 1) \, w \) . $この$z$を$v_1 times dots.h times v_(n - 1)$と書いて，$v_1 \, dots.h \, v_(n - 1)$のクロス積またはベクトル積という．

]
#proof[
  $phi : V arrow.r bb(R)$を$phi \( w \) colon.eq det thin \( v_1 \, dots.h \, v_(n - 1) \, w \)$で定めれば，$phi$は線型写像なので，$phi in V^(\*)$である．したがって@双対空間は縦ベクトル
  の結果より任意の$w in V$に対して$phi \( w \) = chevron.l z \, w chevron.r$を充たす$z$が一意的に存在する．
...
#prop[
  ...
  + $chevron.l x \, x times y chevron.r = 0$ならびに$chevron.l y \, x times y chevron.r = 0$である．
...
#proof[
  ...
  + $det thin \( x \, y \, z \)$は$x \, y \, z in bb(R)^3$の張る平行六面体の体積であった．ここで，$z = x times y \/ parallel x times y parallel$の場合を考えると，$\| det thin \( x \, y \, z \) \| = parallel x times y parallel$である．$chevron.l x \, x times y chevron.r = chevron.l y \, x times y chevron.r = 0$をすでに見たので，$parallel x times y parallel$は$x$と$y$が張る平行四辺形の面積である．
...
#defi[
  $T_p M$の内積$g \( p \) = g_p : T_p M times T_p M arrow.r bb(R)$を，$g_p \( v \, w \) colon.eq chevron.l v \, w chevron.r_p$によって定める（ただし，$chevron.l dot.op \, dot.op chevron.r_p$は$T_p bb(R)^n$の標準内積）．この$g$を$M$の
  Riemann
...
#thm[
  ...
  $ integral_M (partial_1 F^1 + partial_2 F^2 + partial_3 F^3) d V = integral_(partial M) chevron.l F \, n chevron.r d A $が成り立つ．

]
#proof[
  $omega = F^1 d y and d z + F^2 d z and d x + F^3 d x and d y$としたときの
  Stokes
  の定理である．実際，$n^1 d A = d y and d z$などより$chevron.l F \, n chevron.r d A = omega$がわかる．
...
#thm[
  ...
  $ integral_M chevron.l nabla times F \, n chevron.r d A = integral_(partial M) chevron.l F \, T chevron.r d s $が成り立つ．

]
#proof[
  $omega = F_1 d x + F_2 d y + F_3 d z$としたときの Stokes
  の定理である．実際，$n^1 d A = d y and d z$などより$chevron.l nabla times F \, n chevron.r d A = d omega$がわかり，$d s \( T \) = 1$より$T^1 d s = d x$などがわかるので，$omega = chevron.l F \, T chevron.r d s$が従う．
